...
- `overloadCount` and `runtimeArity` stay `null` until a pinned browser probe records the generated JavaScript call forms. The 56 audited bindings comprise the probed `AKAZE`, `AgastFeatureDetector`, `FastFeatureDetector`, `GFTTDetector`, and `KAZE` instance methods plus the core `getOptimalDFTSize` free function. Each records one generated form; getters have arity zero, setters and `getOptimalDFTSize` have arity one. Doxygen C++ and Python forms alone do not resolve either field.
