...
- `overloadCount` and `runtimeArity` stay `null` until a pinned browser probe records the generated JavaScript call forms. The 45 audited non-enum `AKAZE`, `AgastFeatureDetector`, `FastFeatureDetector`, `GFTTDetector`, and `KAZE` instance methods record one generated form each, with arity zero for getters and one for setters. Doxygen C++ and Python forms alone do not resolve either field.
