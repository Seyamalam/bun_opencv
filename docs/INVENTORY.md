...
- `overloadCount` is `null` throughout. Doxygen lists C++ and Python forms, but that does not prove the exact overload set generated by Emscripten. A pinned browser probe must record the callable JS forms before a family can be declared complete.
