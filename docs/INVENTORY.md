...
- `overloadCount` and `runtimeArity` stay `null` until a pinned browser probe records the generated JavaScript call forms. The 59 audited bindings comprise the probed `AKAZE`, `AgastFeatureDetector`, `FastFeatureDetector`, `GFTTDetector`, and `KAZE` instance methods plus the core `countNonZero`, `flip`, `getOptimalDFTSize`, and `transpose` free functions. Each records one generated form. Getters have arity zero; setters, `countNonZero`, and `getOptimalDFTSize` have arity one; `transpose` has arity two; and `flip` has arity three. Doxygen C++ and Python forms alone do not resolve either field.
