...
- `overloadCount` and `runtimeArity` stay `null` until a pinned browser probe records the generated JavaScript call forms. The 68 audited families comprise the probed `AKAZE`, `AgastFeatureDetector`, `FastFeatureDetector`, `GFTTDetector`, and `KAZE` instance methods plus the core `cartToPolar`, `countNonZero`, `exp`, `flip`, `getOptimalDFTSize`, `log`, `magnitude`, `polarToCart`, `pow`, `repeat`, `rotate`, `sqrt`, and `transpose` free functions. `cartToPolar` and `polarToCart` each expose two generated overloads through an arity-zero dispatcher. Every other audited family records one generated form. Getters have arity zero; setters, `countNonZero`, and `getOptimalDFTSize` have arity one; `exp`, `log`, `sqrt`, and `transpose` have arity two; `flip`, `magnitude`, `pow`, and `rotate` have arity three; and `repeat` has arity four. Doxygen C++ and Python forms alone do not resolve either field.
